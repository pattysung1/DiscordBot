...
@bot.command(name="stock-data", help="Check previous day stock data of a company.")
async def stock_data(ctx):
    for stock_company in top_stock_companies:
        try:
            ...
            logging.error(e)
            await ctx.send(f"{stock_company} data is not available for now!")
            continue
        os.remove('images/stock_previous_day.png')

    if not top_stock_companies:
        await ctx.send("You haven't tracked any company yet!")
